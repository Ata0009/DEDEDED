require("http").createServer((req, res) => res.end("Its_Ata0009")).listen(process.env.PORT || 8080)
...
const step1 = new MessageEmbed()
  .setColor("cccfff")
  .setTitle("➡️ How to Use Uptimer!")
  .addField(
    "🔗 Get the link", "Our first step is to get the webpage link. You can find the code in the bottom or side of you repl.it(see screenshot below)! If you do not have this link, copy paste this code at the top of your `index.js` and run it again.\n ```https://pastebin.com/HJGhAUZf```"
  )
...
const step3 = new MessageEmbed()
  ...
  .setTitle("➡️ How to Use Uptimer!")
  .addField(
    "💻 Other Commands", "Now that we have added your project, you can use other command such as `projects` `remove` `stats` and `total`. Below Is an image of the remove command!  "
  )
...
const step2 = new MessageEmbed()
  ...
  .setTitle("➡️ How to Use Uptimer!")
  .addField(
    "⭐ Run the command", "Our next step is to runn the command. The syntax of this command is `*add <repl_url>`. If done correcty the bot should give embed saying: ```:white_check_mark: Added Succesfully!``` See Screenshot Below For More details."
  )
